...
public class DeviceController {
    ...
    @GetMapping(path="/{id}")
    public @ResponseBody Iterable<Device> getDeviceById(@PathVariable("id") Integer id){
        return deviceService.getDeviceByIdService(id);
    }

    @PutMapping(path="/{id}")
    public @ResponseBody Device updateDevice(@PathVariable("id") Integer id,@RequestBody Device device){
